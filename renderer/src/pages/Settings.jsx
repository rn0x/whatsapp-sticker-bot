...
// الإعدادات — نموذج مرتّب حسب الفئات مع شرح لكل إعداد.
// كل النصوص معروضة عبر t() وبالتالي مترجمة بالكامل (عربي/إنجليزي).
// options: إمّا نصوص raw، أو { value, label } حيث label مفتاح عربي يُترجم.
const GROUPED = [
  {
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "quota.mode", label: "وضع الحصة", hint: "نافذة متحركة · يوم ثابت · بلا حدود.", options: [
        { value: "rolling_24h", label: "نافذة متحركة 24س" },
        { value: "daily_fixed", label: "يوم ثابت" },
  ...
  },
  {
    ...
    fields: [
      { key: "whatsapp.provider", label: "موفر الاتصال", hint: "wwebjs يمرّ عبر متصفح حقيقي (موصى به لتفادي الحظر).", options: [
        { value: "wwebjs", label: "wwebjs (متصفح حقيقي)" },
      ] },
  ...
  },
  {
    key: "app", title: "عام",
    fields: [
      { key: "app.theme", label: "المظهر", hint: "اختر بين الليلي والنهاري.", options: [
        { value: "dark", label: "ليلي" },
        { value: "light", label: "نهاري" },
      ] },
      { key: "app.language", label: "اللغة", options: [
...
export default function SettingsPage() {
  ...
  async function save(key, value) {
    ...
    if (key === "app.language") { i18n.changeLanguage(value); applyLang(value); }
  }
  ...
  async function factoryReset() {
    if (resetting) return;
    if (!confirm(t("إعادة الضبط الكامل؟ سيُحذف كل شيء نهائياً: المستخدمون، السجلات، الوسائط، النسخ الاحتياطية، وجلسة واتساب. لا يمكن التراجع عنها."))) return;
    setResetting(true);
    try {
      const r = await window.api.invoke("system:factory-reset", { token, confirmWord: resetWord });
      if (r.ok) {
        alert(t("تمت إعادة الضبط الكامل. يعمل النظام الآن كما لو كان جديداً.\nسيُعاد تحميل اللوحة."));
        window.location.reload();
      } else {
        alert(r.error || t("تعذّرت عملية إعادة الضبط"));
        setResetting(false);
      }
    } catch (err) {
      alert(t("تعذّرت عملية إعادة الضبط: {{err}}", { err: err?.message || err }));
      setResetting(false);
    }
  }
  ...
  return (
    <div className="stack">
      {saved && <div className="toast">{t("تم الحفظ")}</div>}
      {GROUPED.map((g) => (
        <section key={g.key} className="card">
          <header className="card-head"><h3>{t(g.title)}</h3></header>
              ...
              return (
                <div key={f.key} className="field">
                  <span className="field-label">
                    {t(f.label)} <small className="key-label" dir="ltr">({f.key})</small>
                  </span>
                    ...
                    {isSel ? (
                      ...
                      >
                        {f.options.map((o) => {
                          ...
                          return <option key={v} value={v}>{t(label)}</option>;
                        })}
                      </select>
                    ) : isBool ? (
                  ...
                  {f.hint && <small className="setting-desc">{t(f.hint)}</small>}
                  {isDirty && <small className="unsaved">{t("غير محفوظ — اضغط حفظ أو Enter")}</small>}
                </div>
              );
            })}
          </div>
      ...
      <section className="card danger-card">
        <header className="card-head"><h3>{t("منطقة الخطر — إعادة الضبط الكامل")}</h3></header>
        <div className="card-body">
          <p className="setting-desc">
            {t("إعادة الضبط الكامل؟ سيُحذف كل شيء نهائياً: المستخدمون، السجلات، الوسائط، قوائم الانتظار، النسخ الاحتياطية، وجلسة واتساب (تسجيل خروج كامل). لا يمكن استرجاع أي شيء بعدها.")}
          </p>
          <div className="num-wrap reset-line">
            <input
              type="text"
              className="reset-input"
              placeholder={t("اكتب RESET للتأكيد")}
              value={resetWord}
            ...
            >
              {resetting ? t("جارٍ إعادة الضبط…") : t("إعادة الضبط الكامل")}
            </button>
          </div>
        </div>
      </section>
