...
export default function Logs() {
  ...
  const load = useCallback(async () => {
    const r = await window.api.invoke("logs:list", { token, level: level === "ALL" ? undefined : level, limit: 500 });
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => (r.message || "").toLowerCase().includes(q) || (r.scope || "").toLowerCase().includes(q));
  }, [rows, term]);

  async function clear() {
    if (!confirm(t("سيتم حذف جميع السجلات. متابعة؟"))) return;
      ...
      <section className="card">
        <div className="card-body">
          <div className="log-view">
            ...
            {filtered.map((r, i) => (
              <div key={r.id ?? i} className={`log-row lvl-${r.level}`}>
                <span className="log-time mono">{r.created_at}</span>
                <span className={`badge badge-${r.level === "ERROR" ? "danger" : r.level === "WARN" ? "warn" : "info"}`}>{r.level}</span>
                <span className="log-logger mono">{r.scope}</span>
