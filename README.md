...
> ملاحظة: حزمة `flatpak` تُبنى تلقائياً في CI. أما `snap` فتتطلب بنية معزولة (snapd/LXD) غير متوفرة في بيئة البناء التلقائية، لذا تُبنى يدوياً عبر `snapcraft` على جهازك أو عبر `pack:snap` محلياً بعد تثبيت `snapcraft`. الحزم الأكثر موثوقية للتثبيت هي `AppImage` و`deb` و`rpm`.
